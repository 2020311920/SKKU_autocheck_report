...
본인은 엑셀은 물론 사실 데이터를 다루는 것은 이번이 처음이며, </br>심지어 개인 프로젝트, 실제로 서비스를 목표로 하는 것은 모두다 처음입니다.</br></br>
그렇기에 해당 리포지토리를 기록용 겸용으로 사용할 것입니다.</br>
...
- **보고서에서 의도하여 강의를 중복할 경우 이를 시각적으로 보여주는 기능 추가**:
  - **사용자 경험 개선**:
    - 보고서 작성 시 의도치 않게 강의를 중복한 경우에 대해 시각적으로 경고를 표시하여 사용자가 실수를 방지할 수 있도록 하는 기능이 추가되었습니다.
